...
library(dplyr)
library(stringr)
# Added for google sheet persitence storage
library(googlesheets4) 

# Google Sheets setup - ADD THIS SECTION
SHEET_ID <- Sys.getenv("GOOGLE_SHEET_ID")  # Replace with your actual sheet ID
SHEET_NAME <-  Sys.getenv("GOOGLE_SHEET_NAME")  # Replace with your actual sheet ID
...
# Function to save rating to Google Sheets
save_rating <- function(animal_name, rating, session_id) {
  ...
  tryCatch({
    ...
    sheet_append(SHEET_ID, rating_data, sheet = SHEET_NAME)
    cat("Successfully saved to Google Sheets\n")
  }, error = function(e) {
    cat("Error appending to sheet:", e$message, "\n")
    # If sheet doesn't exist, create it
    tryCatch({
      cat("Attempting to create new sheet\n")
      # Create new sheet with headers
      sheet_write(rating_data, ss = SHEET_ID, sheet = SHEET_NAME)
      cat("Successfully created new sheet\n")
    }, error = function(e2) {
      cat("Error creating sheet:", e2$message, "\n")
      # If Google Sheets fails, fall back to local CSV
      warning("Google Sheets failed, saving to local CSV: ", e2$message)
...
# Function to read and summarize ratings from Google Sheets
get_ratings_summary <- reactive({
  ...
  tryCatch({
    ...
    cat("Attempting to read from Google Sheets...\n")
    ratings_data <- read_sheet(SHEET_ID, sheet = SHEET_NAME)
    cat("Successfully read", nrow(ratings_data), "rows from Google Sheets\n")
    
    if (nrow(ratings_data) == 0) {
      cat("No data found in Google Sheets\n")
      return(NULL)
    }
    
    # Debug: Print column names and first few rows
    cat("Column names:", paste(names(ratings_data), collapse = ", "), "\n")
    cat("First few rows:\n")
    print(head(ratings_data, 3))
    ...
    cat("Error reading from Google Sheets:", e$message, "\n")
    # Fall back to local CSV if Google Sheets fails
    if (file.exists("ratings_backup.csv")) {
      cat("Falling back to local CSV\n")
      tryCatch({
        ratings_data <- read.csv("ratings_backup.csv", stringsAsFactors = FALSE)
        ...
        cat("Error reading backup CSV:", e2$message, "\n")
        return(NULL)
      })
    }
    cat("No backup CSV found\n")
    return(NULL)
  })
})
