...
# S3 configuration
S3_BUCKET <- Sys.getenv("AWS_STORAGE_BUCKET_NAME")
S3_REGION <-  Sys.getenv("AWS_DEFAULT_REGION")
S3_DUCKDB_KEY <- "test/ratings.duckdb"  # Single DuckDB file on S3

# Function to download DuckDB file from S3, add rating, and upload back
save_rating_to_s3_duckdb <- function(animal_name, rating, session_id) {
  ...
  tryCatch({
    cat("Saving rating to S3 DuckDB file...\n")
    
    # Create local temp file
    local_db <- tempfile(fileext = ".duckdb")
    
    # Check if DuckDB file exists on S3 first
    file_exists_on_s3 <- tryCatch({
      # List objects with the specific key
      objects <- aws.s3::get_bucket(S3_BUCKET, prefix = S3_DUCKDB_KEY, region = S3_REGION)
      # Check if our specific file is in the list
      any(sapply(objects, function(obj) obj$Key == S3_DUCKDB_KEY))
    }, error = function(e) {
      FALSE
    })
    
    if (file_exists_on_s3) {
      # Download existing DuckDB file from S3
      tryCatch({
        aws.s3::save_object(S3_DUCKDB_KEY, S3_BUCKET, local_db, region = S3_REGION)
        cat("Downloaded existing DuckDB file from S3\n")
      }, error = function(e) {
        cat("Error downloading existing file:", e$message, "\n")
        # If download fails, we'll create a new file
        file_exists_on_s3 <- FALSE
      })
    }
    
    if (!file_exists_on_s3) {
      cat("No existing DuckDB file found, creating new one\n")
      # Don't create the file yet - let DuckDB create it
    }
    
    # Connect to local DuckDB file (this will create it if it doesn't exist)
    con <- dbConnect(duckdb::duckdb(), local_db)
    ...
    # Insert new rating
    dbWriteTable(con, "ratings", rating_data, append = TRUE)
    
    # Disconnect
    dbDisconnect(con)
    
    # Upload updated DuckDB file back to S3
    aws.s3::put_object(
      file = local_db,
      object = S3_DUCKDB_KEY,
      bucket = S3_BUCKET,
      region = S3_REGION
    )
    ...
    cat("Successfully saved rating to S3 DuckDB file\n")
    return(TRUE)
    
  }, error = function(e) {
    cat("Error saving to S3 DuckDB:", e$message, "\n")
    return(FALSE)
  })
}
...
# Function to read ratings summary directly from S3 DuckDB file
get_ratings_summary <- reactive({
  # Depend on the trigger to refresh when ratings change
  ratings_trigger()
  
  tryCatch({
    cat("Reading ratings summary from S3 DuckDB file...\n")
    
    # Check if the DuckDB file exists in S3 using get_bucket
    file_exists <- tryCatch({
      ...
      cat("Error checking S3 bucket:", e$message, "\n")
      FALSE
    })
    
    # If file doesn't exist, return NULL
    if (!file_exists) {
      cat("DuckDB file doesn't exist in S3 yet\n")
      return(NULL)
    }
    
    cat("DuckDB file exists in S3, downloading...\n")
    ...
    count <- dbGetQuery(con, "SELECT COUNT(*) as count FROM ratings")
    if (count$count == 0) {
      dbDisconnect(con)
      unlink(local_db)
    ...
    # Fast SQL aggregation directly on the file
    ...
    cat("Error reading from S3 DuckDB:", e$message, "\n")
    return(NULL)
  })
})
  ...
  save_rating <- function(animal_name, rating, session_id) {
    ...
    # Save to S3 DuckDB file
    s3_success <- save_rating_to_s3_duckdb(animal_name, rating, session_id)
